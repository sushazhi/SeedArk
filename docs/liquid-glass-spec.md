...
- **固定高的停靠栏（顶栏 `h-9`、底栏 `h-9`、列表表头 `h-9`）不要加内容**：
  它们是 `h-9` 定高 flex，行内一旦放不下就会换行并被裁掉（内嵌 WebView 视口更窄，
  更容易触发）。要往里面加东西，先确认最窄目标宽度下仍放得下，或把可省略项
  交给更宽的断点；不要靠 `flex-wrap` 兜底——那正是"变成两行"的成因。

### 7.1 长表单面板：桌面双列 / 窄屏单列

设置这类"分节多、单节可能极长"的面板，一律按 **`md`（768px）** 一分为二：
宽屏左侧 `PaneNav` 常驻节导航、右侧只渲染选中的那一节；窄屏退回单列长滚动 +
`Section` 折叠分节。判据与 `useResponsive` 的 `isMobile` 同口径。

- **显示断点必须与 `isMobile` 一致**（都用 `md`）。曾用 `sm` 隐藏导航，导致
  640–767px 之间"导航已出现、右侧仍按移动端长滚动渲染"的两不像布局。
- **同一时刻只渲染一节**，不要靠折叠隐藏其它节——几十个隐藏表单留在渲染树里
  正是长滚动卡顿的来源。
- **状态提升到面板**：草稿、当前节这类跨节共享的状态放在面板组件里
  （见 `useQBDraft`）。挂在节内会随节卸载而丢失，翻页后已改未存的内容就没了。
- 内容超高时用 `ScrollArea`（量父级实测高度）而不是写死 `dvh`：写死会在内容更矮时
  留一大片空白，看起来就是"这一节底下空着半屏"。
...
| 6 | 状态栏窄屏把计数/空间全部 `hidden md:` 隐藏 | 明细补进「详情」浮层（`md:hidden` 分组），窄屏保留硬盘图标与 ↓/↑ 流量数字；仅隐藏「本次会话」文字标签。（曾加过一行窄屏摘要，因底栏定高、内容超宽会折行被裁，已移除；流量则应放在左侧 `ml-auto` 空档内，不要整块隐藏） |

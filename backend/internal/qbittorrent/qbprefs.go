...
var qbSchema = []driver.SettingsSection{
	...
	},
	{
		...
		Fields: []driver.SettingsField{
			fenumStr("torrent_content_layout", "Torrent 内容布局", "Torrent content layout",
			...
			fenumStr("torrent_stop_condition", "Torrent 停止条件", "Torrent stop condition",
			...
			fenumBool("torrent_changed_tmm_enabled", "种子分类变化时", "When Torrent Category changed",
				boolOpt(true, "重新定位 Torrent", "Relocate torrent"),
				boolOpt(false, "切换 Torrent 到手动模式", "Switch torrent to Manual Mode")),
			fenumBool("save_path_changed_tmm_enabled", "默认保存路径变化时", "When Default Save Path changed",
				boolOpt(true, "重新定位受影响的 Torrent", "Relocate affected torrents"),
	...
	},
	{
		...
		Fields: []driver.SettingsField{
			...
			hint(rng(fi("max_uploads", "全局最大上传槽", "Global maximum number of upload slots", ""), 0, 100000),
				"0 表示不限制", "0 means unlimited"),
			rng(fi("max_uploads_per_torrent", "单种最大上传槽", "Maximum number of upload slots per torrent", ""), 0, 100000),
	...
	},
	{
		...
		Fields: []driver.SettingsField{
			...
			rng(fi("max_active_checking_torrents", "最大活跃检查 Torrent 数", "Max active checking torrents", ""), -1, 10000),
			fb("queueing_enabled", "Torrent 排队", "Torrent Queueing"),
			...
			hint(rng(fi("max_active_torrents", "最大同时活动种子数", "Maximum active torrents", ""), 0, 10000),
				"-1 表示不限制", "-1 means unlimited"),
			fb("dont_count_slow_torrents", "慢速 torrent 不计入限制内", "Do not count slow torrents in these limits"),
			...
			rng(fi("slow_torrent_inactive_timer", "Torrent 非活动计时器", "Torrent inactivity timer", "seconds"), 0, 1<<20),
			...
			hint(rng(fi("max_inactive_seeding_time", "闲置做种时长上限", "Inactive seeding time limit", "minutes"), -1, 1<<20),
				"-1 表示不限制", "-1 means unlimited"),
			fenum("max_ratio_act", "则", "then",
				optInt(ratioStop, "停止 Torrent", "Stop torrent"),
				optInt(ratioRemove, "删除 Torrent", "Remove torrent"),
				optInt(ratioWipe, "删除 Torrent 及所属文件", "Remove torrent and its files"),
				optInt(ratioSuper, "为 Torrent 启用超级做种", "Enable super seeding for torrent")),
	...
	},
	{
		...
		Fields: []driver.SettingsField{
			...
			fs("dyndns_domain", "域名名称", "Domain name"),
	...
	},
	{
		...
		Fields: []driver.SettingsField{
			...
			fb("enable_coalesce_read_write", "合并读写", "Coalesce reads & writes"), fb("enable_piece_extent_affinity", "使用分片区段亲和", "Use piece extent affinity"),
			fb("enable_upload_suggestions", "发送上传分块建议", "Send upload piece suggestions"),
			...
			rng(fi("send_buffer_watermark_factor", "发送缓冲区水位系数", "Send buffer watermark factor", "%"), 50, 500),
			...
			fenum("utp_tcp_mixed_mode", "µTP-TCP 混合模式算法", "µTP-TCP mixed mode algorithm",
				optInt(mixedPreferTCP, "优先使用 TCP", "Prefer TCP"),
				optInt(mixedPeerProp, "按对等节点比重（抑制 TCP）", "Peer proportional (throttles TCP)")),
			...
			fenum("upload_slots_behavior", "上传槽位行为", "Upload slots behavior",
			...
			hint(rng(fi("stop_tracker_timeout", "停止时等待 Tracker 响应的超时", "Stop tracker timeout", "sec"), 0, 1<<20),
				"0 表示禁用", "0 disables it"),
			rng(fi("peer_turnover", "对等节点进出断开百分比", "Peer turnover disconnect percentage", "%"), 0, 100),
			rng(fi("peer_turnover_cutoff", "对等节点进出阈值百分比", "Peer turnover threshold percentage", "%"), 0, 100),
			rng(fi("peer_turnover_interval", "对等节点进出断开间隔", "Peer turnover disconnect interval", "sec"), 0, 3600),
			rng(fi("request_queue_size", "单一对等节点的最大未完成请求", "Maximum outstanding requests to a single peer", ""), 1, 1<<20),
